...
public class PGS_214288 {
    class Solution {
        ...
        public int solution(int k, int n, int[][] reqs) {
            ...
            int[][] total = new int[k+1][n-k+2];
            for(int i = 1; i <= k; i++){
                for(int j = 1; j <= n - k + 1; j++){
                    PriorityQueue<Integer> que = new PriorityQueue<>();
                    for(Node nk : list.get(i)){
                        if(que.size() < j){
                            que.offer(nk.start + nk.time);
                        }else{
                            int min = que.poll();
                            int wait = min - nk.start;
                            if(wait > 0){
                                total[i][j] += wait;
                                que.offer(min + nk.time);
                            }else{
                                que.offer(nk.start + nk.time);
                            }
                        }
            ...
            answer = dfs(n-k+1, total, 1, k);
            return answer;
        }//solution end

        public static int dfs(int remain, int[][] wait, int depth, int k){
            if(depth > k){
                return 0;
            }
            int sum = 100000000;
